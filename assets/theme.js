...
(function(){
  ...
  theme.CartForm = (function() {
    ...
    function CartForm(form) {
      ...
      this.netTotal = form.querySelector(selectors.netTotal);
      this.taxPaid = form.querySelector(selectors.taxPaid);
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function(html) {
        ...
        let tax = subtotal / 1.2

        if (this.taxPaid) {
          this.taxPaid.innerHTML = theme.Currency.formatMoney((subtotal - tax), theme.settings.moneyFormat);
          this.netTotal.innerHTML = theme.Currency.formatMoney((subtotal - (subtotal - tax)), theme.settings.moneyFormat);
        }
  ...
  theme.Collection = (function() {
    ...
    Collection.prototype = Object.assign({}, Collection.prototype, {
      ...
      initFilters: function() {
        ...
        const accordians = document.querySelectorAll('.filter-form .collapsible-trigger__layout span')

        removeTags.forEach((tag) => {
          let filter = tag.getAttribute('data-filter')
          let filteredBy = tag.querySelector('a').innerHTML
          let removeLink = tag.querySelector('a').getAttribute('href')

          accordians.forEach((span) => {




            if (filter == span.innerHTML) {
              span.innerHTML = filteredBy
              let anchor = document.createElement('a')
              anchor.setAttribute('href', removeLink)
              let button = span.parentNode
              let wrap = span.parentNode.parentNode.parentNode
              ...
              wrap.querySelector('button').classList.add('disabled')
              span.classList.add('tag', 'tag--remove', 'tagged-filter')  
              
              wrap.querySelector('.collapsible-content--all').classList.remove('is-open')
            }    
        ...
        if (parent.classList.contains(classes.activeTag)) {
          ...
          if (el.closest('li').classList.contains(classes.removeTagParent)) {
            ...
            document.querySelectorAll(selectors.activeTagList).forEach(list => {
              ...
              newTag.classList.add('tag', 'tag--remove', 'test');
              newTagLink.classList.add('btn', 'btn--small');
              newTagLink.innerText = textEl.innerText;
              newTag.appendChild(newTagLink);
              newTagLink.setAttribute('data-filter', el.getAttribute('name').replace('filter.p.m.custom.', ''))
      ...
      onFormSubmit: function(evt) {
        ...
        el.closest('.collection-sidebar__group').querySelector('.collapsible-trigger__layout--inline span').innerHTML = el.value
      },
